...
public class TileEntityDimChestRenderer extends TileEntitySpecialRenderer {
	...
	private static ResourceLocation RES_LINK = new ResourceLocation("dimchest", "textures/entity/chest/link.png");
	private static ResourceLocation RES_DIM = new ResourceLocation("dimchest", "textures/entity/chest/dim.png");
	...
	public void renderDimChest(TileEntityDimChest tileentitydimchest, double par2, double par4, double par6, float par8) {
		...
		if (tileentitydimchest.hasWorldObj()) {
			metadata = tileentitydimchest.getBlockMetadata();
			orient = tileentitydimchest.orient;
		}
		if (metadata == 0) {
			this.bindTexture(RES_LINK);
		}
		if (metadata == 1) {
			this.bindTexture(RES_DIM);
		}
		GL11.glPushMatrix();
